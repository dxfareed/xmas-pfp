...
async function uploadToPinata(imageData: string, fid: number) {
  ...
  form.append("file", imageBlob, `XmasPFP_${fid}_${Date.now()}.png`);
  form.append("pinataMetadata", JSON.stringify({ name: `Xmas PFP Image for FID ${fid}` }));
  ...
  const pinMetadataResponse = await axios.post("https://api.pinata.cloud/pinning/pinJSONToIPFS", {
    ...
    pinataMetadata: { name: `XmasPFP_Metadata_${fid}_${Date.now()}.json` },
